...
void MainLoop::handler_resting() {
    ...
    Debug::dprint(*tourist, "Woke up. Requesting a pony suit");
    ...
    Debug::dprint(*tourist, "Changing state to WAIT_PONY");
...
void MainLoop::handler_boarded() {
    if (tourist->is_capitan()) { // TODO: Sometimes he can be that last passenger
        ...
        Debug::dprintf(*tourist, "I'm a captain of %d, waiting for the submarine to get full", tourist->my_submarine_id.load());
        ...
        if (tourist->get_boarded_on_my_submarine_size() == 1) {
            Debug::dprintf(*tourist, "Submarine %d is full and I'm alone", tourist->my_submarine_id.load());
        } else {
            Debug::dprintf(*tourist, "Submarine %d is full, asking for permission to start the travel", tourist->my_submarine_id.load());
            ...
            if (tourist->was_submarine_deadlock_detected.load()) {
                ...
                Debug::dprintf(*tourist, "Informing passengers on %d that submarines are leaving not full", tourist->my_submarine_id.load());
            } else {
                msg_type = Packet::DEPART_SUBMAR;
                Debug::dprintf(*tourist, "Informing passengers on %d that the submarine is leaving", tourist->my_submarine_id.load());
            }
        ...
        if (tourist->get_and_clear_is_ack_travel_queued()) {
            Debug::dprintf(*tourist, "Sending queued ACK_TRAVEL to the captain of %d", tourist->my_submarine_id.load());
...
void MainLoop::handler_travel() {
    Debug::dprintf(*tourist, "Going on a journey on the submarine %d", tourist->my_submarine_id.load());
    if (tourist->is_capitan()) {
        int travel_time = random_travel_time();
        Debug::dprintf(*tourist, "Journey on %d will take %d seconds", travel_time);
        std::this_thread::sleep_for(std::chrono::seconds(travel_time));
        // last_cond_mutex = tourist->ack_travel_condition.mutex_lock();
        int on_my_submarine = tourist->get_boarded_on_my_submarine_size();
        Debug::dprintf(*tourist, "Journey on %d has ended, informing passengers", tourist->my_submarine_id.load());
